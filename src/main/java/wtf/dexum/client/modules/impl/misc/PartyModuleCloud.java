...
public final class PartyModuleCloud extends Module {
    ...
    public Map<String, PartyMember> getPartyMembers() {
    ...
    public static class PartyMemberData {
        ...
        public PartyMemberData(String uuid, String name, double x, double y, double z, String dim) {
            this.uuid = uuid;
            this.name = name;
            this.x = x;
            this.y = y;
    ...
    public java.util.List<PartyMemberData> getPartyMembersData() {
        java.util.List<PartyMemberData> list = new java.util.ArrayList<>();
        for (PartyMember m : members.values()) {
            list.add(new PartyMemberData(m.uuid, m.name, m.x, m.y, m.z, m.dim));
        }
        return list;
    }
    ...
    public void onUpdate(EventGameUpdate event) {
        ...
        tickCounter++;
        if (tickCounter >= 10) { // Раз в 0.5 секунды (10 тиков)
            tickCounter = 0;
            sendPosition();
        }
    }
    ...
    public void onRender3D(EventRender3D event) {
        ...
        renderWaypoints(event.getMatrix());
    }

    private void renderWaypoints(MatrixStack matrices) {
        if (client.player == null) return;
        ...
        for (int layer = 0; layer < 3; layer++) {
            ...
            for (int i = 0; i <= 72; i++) { // Увеличено с 36 до 72 для гладкости
                ...
                buffer.vertex(matrices.peek().getPositionMatrix(), (float) dx, 0, (float) dz)
                      .color(0, 200, 0, alpha); // Ярко зелёный RGB
            }
